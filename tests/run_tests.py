...
if __name__ == "__main__":
    ...
    for trace_config in test_traces:
        ...
        if "omit_original" in trace_config and trace_config["omit_original"]:
            omit_original = True
            out_trace_file = trace_config["trace_file"].replace(
                ".prv", ".omit_original.prv"
            )

        print(trace_config["trace_file"])
        test_with_parameters(
            f'tests/traces/{trace_config["trace_file"]}',
            f"tests/out_traces_per_mc/{out_trace_file}",
            f'configs/{trace_config["config"]}',
            trace_config["precision"],
            trace_config["nnodes"],
            trace_config["nsockets"],
            trace_config["nmcs"],
            per_socket=False,
            fail_fast=args.fail_fast,
            no_tests=args.no_tests,
            omit_original_trace=omit_original,
        )
        print("\n")
    ...
    for trace_config in test_traces:
        ...
        test_with_parameters(
            f'tests/traces/{trace_config["trace_file"]}',
            f"tests/out_traces_per_socket/{out_trace_file}",
            f'configs/{trace_config["config"]}',
            trace_config["precision"],
            trace_config["nnodes"],
            trace_config["nsockets"],
