...
def prv_to_df(
    ...
    if file_mb > 100:
        ...
        rand_lines = sorted(
            np.random.choice(num_lines, size=undersample_n_lines, replace=False),
            reverse=True,
        )
    ...
    with open(trace_file_path) as f:
        ...
        df = df[
            (pd.isnull(df[metric_keys]).any(axis=1))
            | (df[metric_keys] >= 0).all(axis=1)
...
def get_dash_table_rows(rows_info: list):
    ...
    return [
        html.Tr([html.Td(row["label"]), html.Td(row["value"])])
        for _, row in rows_info.items()
    ]
