...
def plotCARM(df, peak_bw_gbs, peak_flopss, cache_bw, markers_color, markers_transparency, labels, stress_score_scale,
             graph_title=''):
    ...
    dots_fig = curve_utils.get_roofline_markers_dots_fig(df, x_data, y_data, markers_color, stress_score_scale,
                                                         markers_transparency);
    fig.add_trace(dots_fig)

    # Add a roofline line to the figure
    fig.add_trace(
        go.Scatter(x=[0, peak_flopss / peak_bw_gbs, max_x_value], y=[0, peak_flopss, peak_flopss], mode='lines',
    ...
    for i in range(len(cache_bw)):
        ...
        fig.add_trace(go.Scatter(x=[0, cache_elbow], y=[0, peak_flopss], mode='lines',
                                 line=dict(color='black', width=2, dash=dash_type[i]),
                                 hoverlabel=dict(namelength=0),
