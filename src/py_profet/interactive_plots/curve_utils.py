    ...
    if color == 'stress_score':
        dots_fig = go.Scatter(x=x_data, y=y_data, mode='markers', showlegend=False, marker=dict(
            ...
            cmax=stress_score_scale['max'],
                              ...
                              customdata=df[['node_name', 'socket', 'mc', 'bw', 'lat', 'stress_score']],
                              text=df['timestamp'])
    else:
        dots_fig = go.Scatter(x=x_data, y=y_data, mode='markers', showlegend=False,
                              marker=dict(size=5, opacity=opacity, color=color),
                              xaxis='x',
                              yaxis='y',
                              ...
                              customdata=df[['node_name', 'socket', 'mc', 'bw', 'lat']], text=df['timestamp'])
